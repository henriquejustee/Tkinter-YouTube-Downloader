...
def get_urls_from_playlist(playlist: Playlist) -> List[str]:
    urls = []
    for url in playlist:
        urls.append(url)
...
def make_dir_for_playlist_download(playlist: Playlist, script_path: Path) -> Path:
    playlist_title = playlist.title
    playlist_folder_path = script_path.joinpath(playlist_title)
...
def download_playlist(
    links: List[str], playlist_path: Path, only_audio: bool = False
) -> None:
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:

        if only_audio:
            futures = {
                executor.submit(download_audio, link, playlist_path): link
                for link in links
            }
        else:
            futures = {
                executor.submit(download_video, link, playlist_path): link
                for link in links
            }
